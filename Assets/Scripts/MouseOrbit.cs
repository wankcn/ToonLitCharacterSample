    ...
    void LateUpdate()
    {
        ...
        if (targetObj && targetFocus)
        {
            ...
            if (DraggingObject)
            {
                ...
                {
                    req_ObjRotateSpeed += (Input.GetAxis("Mouse X") * objRotateSpeed * 0.02f - req_ObjRotateSpeed) *
                                          Time.deltaTime * 10;
                }
                else
                {
            ...
            if (surfaceColliders != null && surfaceColliders.Length > 0)
            {
                ...
                {
                    if (surfaceCollider.Raycast(new Ray(transform.position - vdir * bounds_MaxSize, vdir), out hitInfo,
                            Mathf.Infinity))
                    {
                        reqDistance = Mathf.Max(Vector3.Distance(hitInfo.point, targetFocus.position) + distance,
                            reqDistance);
                        surfaceFound = true;
                    }
                }
